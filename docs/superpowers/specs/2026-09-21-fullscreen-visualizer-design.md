...
- 卷帘：
  ...
  - `int rollBlocks(const Score &, const Timeline &, uint32_t elapsedMs,
    ...
    宽度给（一屏最多画得下的方块数：240px 宽、一个方块至少 1px → **240**）。
    真装不下时（方块不到 1px 的密度）**保留以「现在」为中心的那一段**，
    不能填满前 cap 个就收手 —— 300 BPM 的 0.125 拍音符是 25ms 一个，
    6000ms 窗口里 240 个方块、竖线左边就有 80 个，按时间顺序填 64 个槽会在
    竖线左边就填满：正在响的音被整个挤掉、竖线右边一片空白
...
`static RollBlock buf[240]`（固定容量，不每帧分配；240 = 效果区宽度，
约 3.8KB 静态 RAM）。
...
- BPM 极端值：拍点相位用 uint32 毫秒算，不溢出。实机能调到的范围是
  **40~300**（`main.cpp` 把 `gBpm` 钳在 40..300、步长 5），所以上机只走
  这两端；更低的值（20）和非法值（0 / 负数）只可能来自文件里的头部行，
  由 `beatPhase` 的 native 单测覆盖（`bpm <= 0` 退回 120，不除零）

## 7. 测试策略

- **native 单测（lib/vizmodel 全覆盖）**：音高→柱下标的边界与钳制、
  包络在时值内单调不增且不为负、卷帘坐标（过去/当前/未来分类、滚动方向、
  音域归一化含单半音退化、**方块数超过 cap 时只填 cap 个、不越界写、
  且保留段以「现在」为中心（当前音一定在缓冲里、竖线两侧都有方块）**）、
