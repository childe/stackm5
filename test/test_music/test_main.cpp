...
// 测试用的小助手：省掉每次手数字符串长度
static music::Score P(const char *text)
{
...
{
    music::Score s = P("1 5 1' 1,");

    TEST_ASSERT_EQUAL_size_t(4, s.notes.size());
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 261.63f, music::noteToFreq(s.notes[0], s.header));
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 392.00f, music::noteToFreq(s.notes[1], s.header));
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 523.25f, music::noteToFreq(s.notes[2], s.header));
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 130.81f, music::noteToFreq(s.notes[3], s.header));
}
...
{
    music::Score s = P("1=F 4/4 120\n1 5");

    TEST_ASSERT_EQUAL_size_t(2, s.notes.size());
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 349.23f, music::noteToFreq(s.notes[0], s.header));
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 523.25f, music::noteToFreq(s.notes[1], s.header));
}
...
void test_rest_has_no_frequency(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(1, s.notes.size());
    TEST_ASSERT_EQUAL_FLOAT(0.0f, music::noteToFreq(s.notes[0], s.header));
}
...
{
    music::Score s = P("1=C 4/4 120\n3 3 3- 5");
    music::Timeline t = music::buildTimeline(s);
...
void test_index_at_walks_the_timeline_then_ends(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, music::indexAt(t, 0));
    TEST_ASSERT_EQUAL_INT(0, music::indexAt(t, 499));
    TEST_ASSERT_EQUAL_INT(1, music::indexAt(t, 500));
    TEST_ASSERT_EQUAL_INT(2, music::indexAt(t, 1200));
    TEST_ASSERT_EQUAL_INT(-1, music::indexAt(t, 1500));
    TEST_ASSERT_EQUAL_INT(-1, music::indexAt(t, 99999));
}
...
void test_header_prefix_len(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(12, music::headerPrefixLen("1=C 4/4 120\n3 3", 15));

    // 没有头部行
    TEST_ASSERT_EQUAL_size_t(0, music::headerPrefixLen("3 3", 3));

    // 只有头部行、末尾没有换行
    TEST_ASSERT_EQUAL_size_t(11, music::headerPrefixLen("1=C 4/4 120", 11));

    // 空文本
    TEST_ASSERT_EQUAL_size_t(0, music::headerPrefixLen("", 0));
}
